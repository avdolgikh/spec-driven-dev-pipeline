...
### 1b. Remove `format_command` / `_run_format` / `_format_hint` `[ ]`
...
### 1c. Remove `uv` fallback logic in `_run_pytest_gate` `[ ]`
...
### 1d. Revert curly-quote prompt additions `[ ]`
...
### 2b. Consolidate task-term matching `[ ]`
...
### 2c. Clean up `_ensure_tests_stage_effect` `[ ]`
...
### 2d. Remove `hasattr(time, "sleep")` guard `[ ]`
...
### 3a. Deduplicate test boilerplate `[ ]`
...
1. OpenCode (REQ-3)
2. Gemini (REQ-4)
3. Claude (REQ-2) -- **separate session, token-expensive**
